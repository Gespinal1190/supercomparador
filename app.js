document.addEventListener("DOMContentLoaded", () => {
  ...
  async function buscarProductos() {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/productos?q=${encodeURIComponent(q)}&super=${encodeURIComponent(superFilter)}`);
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({}));
        throw new Error(errorData.details || "Error en servidor");
      }
